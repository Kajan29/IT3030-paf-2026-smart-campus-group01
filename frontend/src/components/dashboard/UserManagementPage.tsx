...
import { adminUserService, type UserResponse, type CreateStaffRequest } from "@/services/adminUserService";
...
const AddStaffModal = ({ onClose, onSuccess }: AddStaffModalProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSuccess({ password: response.data.data.defaultPassword });
      setTimeout(() => {
        onSuccess();
        onClose();
      }, 3000);
