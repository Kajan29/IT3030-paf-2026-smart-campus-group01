  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const currentBuilding = snapshot.buildings.find((entry) => entry.id === buildingId) || null;
        const floorData = snapshot.floors
          .filter((entry) => entry.buildingId === buildingId)
          .sort((a, b) => a.floorNumber - b.floorNumber);
        const roomData = snapshot.rooms.filter((entry) => entry.buildingId === buildingId);
  ...
  if (loading) {
    return (
      <AdminLoadingState
        ...
        subtitle="Getting occupancy insights, floor coverage, and room distribution."
      />
    );
  }
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-4 mt-3 text-sm text-muted-foreground">
                ...
                {(building.openingTime || building.closingTime) && (
                  <span className="flex items-center gap-1.5"><CalendarDays size={14} /> {building.openingTime || "08:00"} - {building.closingTime || "18:00"}{building.closedOnWeekends ? " · Weekends closed" : ""}</span>
                )}
                {building.createdBy && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {rooms.map((room) => (
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mt-0.5">{room.code} â€¢ {room.type}</p>
